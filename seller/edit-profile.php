...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'] ?? '')) {
        ...
        // Business fields (only if business seller)
        $busName   = trim($_POST['bus_name']    ?? '');
        $busTaxID  = trim($_POST['bus_taxid']   ?? '');
        $busRegNum = trim($_POST['bus_regnum']  ?? '');
        $busType   = trim($_POST['bus_type']    ?? '');
        $busPhone  = trim($_POST['bus_phone']   ?? '');
        $busAddr   = trim($_POST['bus_address'] ?? '');
        ...
        if ($isBusiness) {
            if (!$busName)   $errors[] = 'Business name is required.';
            if (!$busTaxID)  $errors[] = 'Tax ID is required.';
            if (!$busRegNum) $errors[] = 'Registration number is required.';
            if (!$busType)   $errors[] = 'Business type is required.';
            if (!preg_match('/^[0-9]{11}$/', $busPhone)) $errors[] = 'Business phone must be exactly 11 digits.';
            if (!$busAddr)   $errors[] = 'Business address is required.';
        }
...
<style>
    ...
    .ep-card-title svg { width: 16px; height: 16px; stroke: currentColor; fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; color: var(--muted); }
    ...
    .form-group input:disabled { background: var(--bg); color: var(--muted); cursor: not-allowed; }
    ...
    .readonly-note svg { width: 13px; height: 13px; stroke: currentColor; fill: none; stroke-width: 2; }
...
<div class="ep-wrap">
    <a href="seller.php" class="ep-back">
        <svg viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6"/></svg>
        Back to seller profile
    </a>

    <h1 class="ep-title">Edit Seller Profile</h1>
    ...
    <form method="POST" action="edit-profile.php">
        ...
        <div class="ep-card">
            ...
            <p class="readonly-note">
                <svg viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                Seller type and status are managed by eBay administrators.
            </p>
        </div>
        ...
        <div class="ep-card">
            ...
            <p style="font-size:12px;color:var(--muted);margin-bottom:16px;">Leave both fields blank to keep your current password.</p>
        ...
        <div class="btn-row">
            ...
            <a href="seller.php" class="btn-cancel">Cancel</a>
        </div>
    </form>
</div>
